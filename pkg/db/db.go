...
func (d *DB) WriteMigrationVersion(id int64, hash string) error {
	if id == 0 {
...
func (d *DB) GetMigrationVersions() ([]*model.Migration, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&mig.ID, &mig.Version, &mig.Changes, &mig.Hash, &mig.Applied, &mig.ErrorMessage, &mig.Failed, &mig.CreatedAt, &mig.Status)
		if err != nil {
			return nil, err
		}
